...
13. **Handle Fantasy Updates**:
    - **If the round is NOT finished**: Ask the user: *"Would you like to update the fantasy points database now as well?"* Run the update only if they give manual approval.
    - **If the round IS finished** (i.e. the last match of the round was just completed and saved): Do **NOT** ask for approval. Automatically execute the **Fantasy Points Database Updates** procedure below.
